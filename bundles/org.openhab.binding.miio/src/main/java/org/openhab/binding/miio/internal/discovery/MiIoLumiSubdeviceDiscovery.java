...
import org.openhab.binding.miio.internal.json.GatewayDeviceItem;
 ...
 */
@NonNullByDefault
@Component(service = DiscoveryService.class, configurationPid = "discovery.miio.lumisubdevice")
public class MiIoLumiSubdeviceDiscovery extends AbstractDiscoveryService 
    ...
    private final Logger logger = LoggerFactory.getLogger(MiIoLumiSubdeviceDiscovery.class);
    @Nullable
    private CloudConnector cloudConnector;
    private Map<String, String> cloudDevices = new ConcurrentHashMap<>();
    private @Nullable Configuration miioConfig;

    protected @Nullable MiIoGatewayHandler bridgeHandler;

    @Activate
    public MiIoLumiSubdeviceDiscovery()
             {
    ...
    protected void startBackgroundDiscovery() {
        logger.debug("Start Xiaomi Mi IO Lumi Subdevice background discovery with cloudDiscoveryMode: {}", getCloudDiscoveryMode());
    ...
    protected void stopBackgroundDiscovery() {
        logger.debug("Stop Xiaomi  Mi IO Lumi Subdevice background discovery");
        final @Nullable ScheduledFuture<?> miIoDiscoveryJob = this.miIoDiscoveryJob;
    ...
    protected void startScan() {
        ...
        logger.info("AutoDiscovery MIIO Lumi Subdevice: StartScan...");
        discover();
    }
    ...
    private void discover() {
        ...
        // }
        logger.info("AutoDiscovery MIIO Lumi Subdevice: Discover...");
    ...
    public void setThingHandler(@Nullable ThingHandler handler) {
        if (handler instanceof MiIoGatewayHandler) {
            bridgeHandler = (MiIoGatewayHandler) handler;
    ...
    public void createDisvoceryResult(GatewayDevicesList deviceList) {
        ...
        for (GatewayDeviceItem device : deviceList.result) {
            String id = device.did;
            if (MiIoDevices.getType(device.model).getThingType().equals(THING_TYPE_UNSUPPORTED)) {
                logger.info("Discovered gateway subdevice, but ignored because not supported: {} {}", id, device);
            }
            else {        
                String uid = device.did.replace(".", "_");        
                submitDiscovery(device.model, uid, device.did, MiIoDevices.getType(device.model).getDescription());
            }
    ...
    private void submitDiscovery(String model, String uid, String did, String description) {
                ThingTypeUID thingType = MiIoDevices.getType(model).getThingType();
                ThingUID thingUid = new ThingUID(thingType, bridgeHandler.getThing().getUID(), uid);
                
                DiscoveryResultBuilder discoveryResultBuilder = DiscoveryResultBuilder.create(thingUid)
                        .withBridge(bridgeHandler.getThing().getUID())
                        .withRepresentationProperty(PROPERTY_DID)
                        .withProperty(PROPERTY_DID, did)
